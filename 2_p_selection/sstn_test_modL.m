function [STFT,SST1,SST2,SST3,SST4,omega,omega2,omega3,tau2,tau3,phi22p,phi23p,phi24p,omega4] = sstn_test_modL(s,gamma,sigma,ft,bt)
%-------------------------------------------------------------------------------------
% sstn : computes the STFT of a signal and different versions of synchrosqueezing/reassignment described in:
% D.-H. Pham and S. Meignen "High-Order Synchrosqueezing Transform for Multicomponent Signals Analysis - With an Application to Gravitational-Wave Signal", IEEE T SIGNAL PROCES, 2016
% With modifications by J.M. Miramont and M. A. Colominas.
%----------------------------------------------------------------------------------
% INPUTS:
...
% Window definition
%t = -0.5:1/n:0.5-1/n;
% g =  1/sigma*exp(-pi/sigma^2*(t-0.25).^2);
% figure(); plot(s);  hold on; plot(g); plot(ones(1,n)*exp(-4.5)*max(g));
...
vg = zeros(8,nb); % Este consume menos memoria.
vgp = zeros(6,nb);
Y = zeros(7,nb,7);
...
if n == 0
    ...
    if n == 1
       Hn = 2*t; 
        ...
        Hn = 2*t.*hermite_poly(t,n-1) - 2*(n-1)*hermite_poly(t,n-2);
    end
end
